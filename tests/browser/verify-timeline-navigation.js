async page => {
  ...
  try{
    ...
    for(const basis of ['argument','semantic']){
      ...
      const before=await state();await drag('right',110,70);const pan=await state();
      ...
      await drag('left',160,90);const orbit=await state();
      assert(distance(pan.cameraQ,orbit.cameraQ)>.01,'Left drag does not inspect depth');
      assert(distance(pan.root,orbit.root)<1e-6,'Left drag rotates timeline model instead of orbiting camera');
      assert(Math.abs(orbit.roll)<1e-6,'Year axis rolled');
      assert(Math.abs(pan.distance-orbit.distance)<1e-6,'Orbit changes zoom');
      await drag('right',50,30);const repan=await state();assert(distance(orbit.cameraQ,repan.cameraQ)<1e-6,'Pan after orbit rotates camera');
