async page=>{
 ...
 try{
  ...
  for(const protocol of ['openai-chat','anthropic-messages','openai-responses']){
   ...
   const presets=await page.locator('[data-discovery-filter="resultCount"]').allTextContents();assert(presets.slice(0,6).join(',')==='1,10,20,50,100,200','Count presets missing');
   await page.locator('#start-discovery').click();await page.waitForFunction(()=>!window.__discoveryQA.searching,{},{timeout:120000});
   const records=await page.evaluate(()=>window.__discoveryQA.results);assert(records.length===1,'Incorrect count: '+await page.evaluate(()=>window.__discoveryQA.notice));assert(records[0].metadataSource==='OpenAlex','No real source');assert(records[0].language==='en','Language filter failed');
   await page.screenshot({path:`output/playwright/discovery-${protocol}-results.png`});
   if(protocol==='openai-chat'){
    await page.locator('[data-discovery-result]').first().check();await page.locator('#discovery-confirm').click();await page.waitForFunction(()=>!window.__discoveryQA.importing,{},{timeout:120000});
  ...
  await page.locator('[data-discovery-value="custom"]').click();await page.locator('#discovery-result-count').fill('0');await page.locator('#start-discovery').click();assert((await page.locator('.discovery-notice').innerText()).includes('1 to 1000'),'Custom count validation missing');
