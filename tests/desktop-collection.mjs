...
try{
 ...
 const record=await page.evaluate(()=>JSON.parse(localStorage.getItem('litgraph.chat.v2.sample-project.project.')).at(-1));assert.equal(record.status,'done',record.text);assert.equal(record.reviewLedger.length,50);assert.equal(ids.size,50);assert.equal(reviewCalls,7);assert.equal(finalCalls,1);assert.match(record.text,/范围：50 篇/);assert.ok(record.sources.length);assert.deepEqual(errors,[]);
 await page.reload();
