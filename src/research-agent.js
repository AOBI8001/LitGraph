...
import {RESEARCH_ANSWER_STYLE,compactResearchAnswer,asksRetrievalDiagnostics} from './research-answer-style.js';
...
export function researchMessages(nodes, history, question, context = {}, mode = 'quick') {
  ...
  return [
    ...
    { role: 'user', content: JSON.stringify({
      ...
      collection_guidance:collection?'Consider EACH scoped paper. For screening, list evidenced matches; do not infer absence from a retrieval miss. For comparisons, give each paper its own evidence. Keep any necessary uncertainty specific and brief; do not recite coverage bookkeeping.':undefined,
      ...
      recent_conversation: history.filter(item => ['user', 'assistant'].includes(item.role) && (!item.status || item.status === 'done') && !/^无法完成分析：|^Analysis failed:/.test(item.text)).slice(-6).map(item => {const text=item.role==='assistant'?compactResearchAnswer(historyWithoutEvidence(item.text), 'zh', {diagnostics:asksRetrievalDiagnostics(question)}):String(item.text||'');return { role: item.role, content: text.slice(0, policy.mode === 'quick' ? 2000 : 4000), truncated: text.length > (policy.mode === 'quick' ? 2000 : 4000) };}),
