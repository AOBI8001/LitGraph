...
export function researchModePolicy(mode) {
  return normalizeResearchMode(mode) === 'expert'
    ...
    : { mode: 'quick', priority: 'speed', evidenceBudget: 28000, instruction: 'Quick is a provider-independent speed/depth preference, not a model identity or merely a short-answer instruction. Prioritize a timely final answer: focus analysis on the most relevant evidence and the current question, avoid unnecessary exhaustive exploration, and state the key result directly. For a typical conceptual question, aim for 350–600 Chinese characters or 220–380 English words, excluding source previews and follow-ups. Explain the conclusion, 2–4 substantive evidence-backed points, and any essential limitation. For multi-paper comparisons, cover the requested papers across the relevant method dimensions with source citations; expand beyond this range when needed rather than omit papers. Simple factual questions can be shorter, and explicit user length requests take precedence. Do not pad unsupported detail, repeat the question, provide a generic preamble, or recite retrieval bookkeeping. Keep the same evidence and uncertainty standards as expert mode; never sacrifice accuracy or replace missing evidence with guesses.' };
}
