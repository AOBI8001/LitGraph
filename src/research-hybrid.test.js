...
test('hybrid selects explicit papers, keeps whole chunks and records fallback',async()=>{
 ...
 const result=await hybridEvidence(docs,q,plan,{topK:4,embed:async texts=>texts.map(()=>[1,0])});assert.equal(result.retrieval.method,'bm25+dense+rrf+feature-rerank');assert.deepEqual(new Set(result.evidence.map(e=>e.documentId)),new Set(['0','1']));assert.ok(result.evidence.every(e=>!e.truncated));assert.match(evidenceLocations('Fact [E1]',result.evidence,'en'),/PDF page 1/);
 const fallback=await hybridEvidence(docs,q,plan,{embed:async()=>{throw Error('missing model');}});assert.match(fallback.retrieval.warning,/keyword-only fallback/);
});
...
test('quick retains full-corpus dense retrieval and cancellation is propagated',async()=>{
 ...
 const result=await hybridEvidence(docs,'reliability',quickQueryPlan('reliability'),{embed,...retrievalPolicy(quickQueryPlan('reliability'),4)});assert.ok(passages>48);assert.ok(result.evidence.length<=10);assert.equal(result.retrieval.denseCandidateCount,result.retrieval.candidateCount);
 await hybridEvidence(docs,'reliability',quickQueryPlan('reliability'),{embed,mode:'expert'});assert.ok(passages>48);
...
test('detailed, numerical and comparison questions retain model rewriting and full evidence budget',()=>{
 ...
 assert.equal(retrievalPolicy({queries:['question']},50).topK,20);
