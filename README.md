...
所有 3D 视图均支持右键拖动或左键拖动空白处平移，画面跟随拖动方向移动。`Shift`＋左键拖动可旋转模型（年份树为有限角度观察）。滚轮用于缩放，方向键与 `WASD` 用于移动镜头，画布内提供操作提示。工具栏还提供节点重命名、位置锁定、适应画布和全屏浏览。
...
**快速模式使用本地双语词汇扩展，省去单独的远程规划调用。** 专家模式保留模型规划；规划格式失败时保留原问题继续检索，并记录降级状态。快速模式为向量召回设置单篇 1.2 秒、多篇 3.5 秒预算；未就绪或超时则使用关键词检索，不等待整库补建向量。这是向量检索预算，不是完整回答耗时承诺。
