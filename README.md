...
1.3.0：快速回答更充分；修复专家回答因追问格式异常而丢失正文的问题。提供 Windows 与 Apple Silicon macOS 安装包。详情见 [更新日志](RELEASE_NOTES.md)。
...
**1.3.0 · Windows 10 / 11 x64 · macOS 13+（Apple M 系列芯片）· MIT 开源**
...
> macOS 版采用临时签名（ad-hoc），尚无 Apple Developer ID 签名及公证。仅在确认来源可信且校验和一致后，按 [Apple 的说明](https://support.apple.com/en-au/102445) 在“系统设置 → 隐私与安全性”批准此应用；不要关闭系统整体安全防护。
...
**大批量导入与续处理（1.2.1）：** 暂停后可切换 API 或外部 Agent，再点击“继续”。软件核对磁盘中的原文和 MD，优先分析已有 MD，之后再处理待转换文件；每个成功阶段分别保存。桌面项目状态由磁盘持久化，不再受浏览器本地缓存约 5 MB 的配额限制。这不代表内存、磁盘或模型上下文没有上限。

历史记录中“继续”与“展开”之间的 **重新绘制画布** 按钮会重新计算布局，并隐藏该次导入中尚未分析的灰色节点；不会删除论文、PDF、MD、分析或历史记录。再次点击“继续”，这些节点恢复显示。折叠历史仍显示获取、转换、分析进度，明细仅在展开时生成。PDF 转换附带本地字体映射资源；扫描件和损坏编码仍可能需要 OCR 或补充可读原文，不会将不可靠文字强行标为转换成功。
...
所有 3D 视图均保留左键拖动空白处旋转模型（年份树为有限角度观察），无需组合键。右键拖动平移画布，画面跟随拖动方向移动。滚轮用于缩放，方向键与 `WASD` 用于移动镜头，画布内提供操作提示。工具栏还提供节点重命名、位置锁定、适应画布和全屏浏览。
...
问题原意始终保留；完整标题用于确定论文范围，检索词去除冗长标题，聚焦实际所问事实。对于需要模型规划的请求，系统结合有限的近期用户问题及明确提及的论文元数据，要求生成两至三条补充查询与最多六个具体词项（兼容旧格式的两层关键词）：
...
**快速模式的简单概念问题使用本地双语词汇扩展，省去单独的远程规划调用。** 精确事实、明确论文、比较题及专家模式使用有截止时间的精简规划；失败时使用本地词汇扩展，保留事实题预算并记录降级状态。快速模式的向量召回预算为简单单篇 1.2 秒、其余 3.5 秒；未就绪或超时则使用关键词检索，不等待整库补建向量。这是向量检索预算，不是完整回答耗时承诺。
...
| 快速 · 本地扩展 · 多篇 | 12 | 12,000 字符 |
| 快速 · 精确事实 / 比较 · 模型规划 | 20 | 28,000 字符 |
| 专家 · 模型规划 | 24 | 42,000 字符 |

上述预算针对证据正文，不等同于整个模型请求的 token 数；必要元数据、规范和有限对话历史另行计入请求。实际证据数量同时受段落长度、可用原文和相关性影响。

1.2.6 对精确事实与比较题增加一轮本地补检：按问题维度及匹配的摘要/开头原文补充最多 6 个完整片段，额外预算不超过 8,400 字符，总预算上限 48,000 字符。补检不再次调用模型，也不现场建立段落向量；它是覆盖启发式，不代表穷尽了全部相关证据。检索词规划最多等待 16 秒，失败仍保留较大的事实题证据预算。简单概念问题继续走一次回答模型的快速路径。
...
历史模型质量评测见 [1.2.6 质量与耗时报告](docs/RAG_BENCHMARK_1.2.6.md)：固定回归集参考证据召回 97.75%、严格答案准确率 100%、后端完整回答中位数 20.60 秒。这不是 1.2.8 新范围覆盖流程的模型质量或耗时测量，也不是所有语料/模型的准确率保证。

1.2.8 的范围覆盖会先确定项目或选中论文范围：概括读取逐篇研究目的卡片；比较/筛选逐篇检索，不竞争全库 Top-K。旧论文在读取时自动补建本地摘录卡片，不额外调用模型。新论文分析在同一次模型请求中提取卡片原句，经源文匹配后保存；没有明确证据的字段不填造。大范围问题按批处理、必要时分层汇总，显示覆盖数量与不确定性。检索缺失不等于事实不存在，卡片中的本地候选摘录也不代表人工语义核验。详见 [两类问答与覆盖说明](docs/RESEARCH_COVERAGE_1.2.8.md)。

新增论文保存为 MD 后，自动排队建立带版本与来源位置的分块向量索引，合并同页同章节的过短段落，并增量保存进度；启动时自动补建旧项目、恢复未完成索引，无需在研究空间点击按钮。提问只使用已就绪的向量，不现场编码整库。修改原文会改变相关分块标识，同一段未变化的编码输入仍可复用已有向量。新的请求按当前项目范围准备候选，隐藏节点不会因存在磁盘缓存而进入回答。
