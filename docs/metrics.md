...
日期按北京时间划分。安装标识不是自然人身份：同一人两台电脑算两个安装；复制整个用户数据目录可能被视为一个安装；删除用户数据后重新使用会产生新标识。离线、关闭统计、网络拦截或修改过的客户端可能导致少计；公开上报接口也不能完全防止伪造请求。这些是产品使用趋势，不是审计级实名人数。
...
3. 查看累计安装及最近 31 个日期的统计；再次点击“查看统计”刷新。
