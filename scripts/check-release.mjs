...
for(const entry of ['projects/','output/','.playwright-cli/','versions/','.env','node_modules/','dist/','*.local.json','release/'])
...
for(const file of tracked){
 assert.doesNotMatch(file,/^(?:projects|output|release|node_modules)\/|\.local\.json$|\.env$/,'Private path tracked');
...
for(const file of ['README.md','README.en.md']){
 ...
 assert.match(markdown,/1\.0/);assert.match(markdown,/5.*10.*20.*50.*100/);
