...
export function createVectorService(root,dataRoot){
 ...
 const request=async(message,signal)=>{
  signal?.throwIfAborted();const active=start();active.ref();
  return new Promise((resolve,reject)=>{const id=randomUUID(),cleanup=()=>signal?.removeEventListener('abort',abort),abort=()=>{pending.delete(id);active.postMessage({cancel:id});cleanup();if(!pending.size)active.unref();reject(signal.reason||Error('Cancelled'));};pending.set(id,{resolve,reject,cleanup});signal?.addEventListener('abort',abort,{once:true});active.postMessage({id,...message});});
 };
 const embed=async(texts,kind,signal)=>{if(!Array.isArray(texts)||texts.length>128||!texts.every(t=>typeof t==='string'&&t.length<=4000)||!['query','passage'].includes(kind))throw Error('Invalid embedding request');return request({texts,kind},signal);};
 embed.index=(operation,data={},signal)=>request({operation,...data},signal);
 embed.start=()=>{try{start();}catch{}};
 embed.close=async()=>{const current=worker;worker=null;for(const task of pending.values()){task.cleanup();task.reject(Error('Vector service closed'));}pending.clear();await current?.terminate();};
 return embed;
}
