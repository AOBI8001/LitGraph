...
const base='https://api.github.com/repos/AOBI8001/LitGraph',tag='v'+version,names=[`LitGraph-Setup-${version}-x64.exe`,'SHA256SUMS.txt'];
...
if(action==='draft'){
 ...
 for(const name of names){const file=`release/${version}/${name}`,info=await stat(file),sha=await hash(file),old=release.assets.find(a=>a.name===name);
  ...
  const asset=await api(url,{method:'POST',headers:{'Content-Type':name.endsWith('.exe')?'application/octet-stream':'text/plain','Content-Length':String(info.size)},body:await openAsBlob(file)});
